...
fn get_image(imageref: magic::ImageReference) -> Vec<u8> {
    println!("Looking at address {:#08X} for image", imageref.addr);
    panic!();
    Vec::new()
}
                ...
                loop {
                    ...
                    if let Ok(command) = command_option {
                        match command { 
                            ...
                            magic::RDPCommand::ShadeTriangle(coefficients, shading) =>  {
                                if !seen {
                                    println!("Drawing a shade triangle with these coefficients: {:#X?}\n{:#X?}", coefficients, shading);
                                    magic::draw_shade_tri(&mut fb_bytes, coefficients, shading);
                                }
                            },
